pipeline {
    ...
    post {
        failure {
            ...
            sh '''
            if [ -n "$(kubectl get deployment ${APP_NAME} -o=jsonpath='{.spec.template.spec.containers[0].image}')" ]; then
                kubectl set image deployment/${APP_NAME} ${APP_NAME}=$(kubectl get deployment ${APP_NAME} -o=jsonpath='{.spec.template.spec.containers[0].image}')
            fi
            '''
        }
    }
