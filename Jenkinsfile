pipeline {
    ...
    stages {
        ...
        stage('Quality Gate') {
            steps {
                script {
                    def qualityGate = sh(script: "curl -u ${Sonar_Token}: ${Sonar_Url}/api/qualitygates/project_status?projectKey=${APP_NAME} | jq -r .projectStatus.status", returnStdout: true).trim()
